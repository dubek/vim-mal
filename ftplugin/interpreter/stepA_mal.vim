runtime! ftplugin/interpreter/readline.vim
runtime! ftplugin/interpreter/types.vim
runtime! ftplugin/interpreter/reader.vim
runtime! ftplugin/interpreter/printer.vim
runtime! ftplugin/interpreter/env.vim
runtime! ftplugin/interpreter/core.vim
...
let g:Repl_env = NewEnv("")

for [k, v] in items(CoreNs)
  call g:Repl_env.set(k, v)
endfor

call g:Repl_env.set("*ARGV*", GetArgvList())

call RE("(def! *host-language* \"vimscript\")", g:Repl_env)
call RE("(def! not (fn* (a) (if a false true)))", g:Repl_env)
call RE("(def! load-file (fn* (f) (eval (read-string (str \"(do \" (slurp f) \")\")))))", g:Repl_env)
call RE("(defmacro! cond (fn* (& xs) (if (> (count xs) 0) (list 'if (first xs) (if (> (count xs) 1) (nth xs 1) (throw \"odd number of forms to cond\")) (cons 'cond (rest (rest xs)))))))", g:Repl_env)
call RE("(def! *gensym-counter* (atom 0))", g:Repl_env)
call RE("(def! gensym (fn* [] (symbol (str \"G__\" (swap! *gensym-counter* (fn* [x] (+ 1 x)))))))", g:Repl_env)
call RE("(defmacro! or (fn* (& xs) (if (empty? xs) nil (if (= 1 (count xs)) (first xs) (let* (condvar (gensym)) `(let* (~condvar ~(first xs)) (if ~condvar ~condvar (or ~@(rest xs)))))))))", g:Repl_env)

" if !empty(argv())
"   try
"     call RE('(load-file "' . argv(0) . '")', g:Repl_env)
"   catch
...
" endif
"
" call REP("(println (str \"Mal [\" *host-language* \"]\"))", g:Repl_env)
"
" while 1
...
"   endif
"   try
"     call PrintLn(REP(line, g:Repl_env))
"   catch
"     call PrintLn("Error: " . v:exception)
"   endtry
" endwhile
